...
class MUSTCData(object):
    ...
    def prepareData(self, data_directory="./", reverse=False):
        pairs = []
        txt_directory = os.path.join(data_directory, l1+"-"+l2+"/data/train/txt")
        wavs_directory = os.path.join(data_directory, l1+"-"+l2+"/data/train/wav")
        features_directory = os.path.join(data_directory, l1+"-"+l2+"/features/train")
